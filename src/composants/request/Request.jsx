  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in formData) {
      if (formData[key] instanceof File) {
        dataToSend.append(key, formData[key]); // Fichier
      } else {
        dataToSend.append(key, formData[key]); // Champ classique
      }
    }
  ...
  return (
    <Form onSubmit={handleSubmit} autoComplete="off" className={className}>
      ...
      <Form.Group as={Col} md={11} className="my-2">
        <Button className="col-12 rounded-5" variant="success" id="btn-submit" type="submit" disabled={loading}>
...
export const TableData = ({url, headerItem, renderRow, code, id, typeReturn }) => {
    ...
    useEffect (() => {
        ...
        const getUser = async () => {
            ...
            try {
                const res = await api.get(url, { params }, {headers : {'Content-Type' : 'application/json'}});
                console.log(res.data.message);
                if (res.data.status === 'success') {
                    ...
                    setMessageSer(res.data.message);
                }
            } catch (error) {
                setMessageSer(error.res?.data?.message || "Erreur lors de la connexion !");
            }
    ...
    switch (typeReturn) {
        case 'Table':
            return(
                <>
                  ...
                  <>
                    <Table responsive hover className="text-center table-sm mt-3 fade-in-box" id="table-default" >
                        <thead>
                            <tr>
        ...
        case "multiReturn" :
          return (
            <>
              {
                datas.length === 0 ? <p className="text-center"><span className="spinner spinner-border mt-2"></span></p> : datas.map((item, index) => (
                  <>
...
export const FormSerch = ({search, setSearch, classSerch}) => {
  return (
      <Row className="justify-content-end fade-in-box">
          <Form.Group className={ classSerch ? classSerch : "my-2 col-md-3"}>
...
export const RechercheReq = ({ url, search, headerItem, renderRow, typeReturn}) => {
  const [messageSer, setMessageSer] = useState('');
  ...
  useEffect(() => {
      ...
      setTimeout( async () => {
          if (search) {
              ...
              const res = await api.get(url, {params : { r : search }}, {headers : {'Content-Type' : 'application/json'}});
              const data = res.data.data;
  ...
  if (typeReturn === "multiReturn") {
    return (
      <>
        {
          resultat.length === 0 ? messageSer === "Aucun résultat trouvé!" ? messageSer : <p className="text-center"><span className="spinner spinner-border mt-2"></span></p> 
      ...
      <>
          {status === "error" ?  messageSer === "Terme de recherche obligatoire" ? "" : 
              ...
              <Row className="bg-white my-2">
                  <Table responsive hover className="text-center mt-3" id="table-default">
                    <thead>
                        <tr>
                            {headerItem.map((item, idx) => (
                                <th className="th" key={idx}>{item}</th>
